...
export default async function HrOnboardingPage({ searchParams }: HrOnboardingPageProps) {
  ...
  const [{ data: profile }, { data: assignments }, { data: signatures }, { data: responses }, { data: signedDocuments }] =
    await Promise.all([
        ...
        <section className="hr-document-stack">
          {typedAssignments.length === 0 ? (
            ...
            typedAssignments.map((assignment) => {
              ...
              const canEditCurrentStep = assignment.status === "pending" && (!activePendingAssignmentId || activePendingAssignmentId === assignment.id);
              ...
              return (
                <article className="doc-card hr-sign-card" key={assignment.id}>
                  <div className="portal-topline" style={{ marginBottom: 12 }}>
                    ...
                    {assignment.status === "signed" ? (
                      <span className="badge">
                        ...
                        Signed {formatDate(signedDocument?.signed_at ?? signature?.signed_at ?? assignment.signed_at)}
                      </span>
                    ) : (
                  ...
                  {formDefinition && assignment.status === "pending" && canEditCurrentStep ? (
                    ...
                    <div className="success-box">Complete the current step above before filling this form. Draft status: {response?.status ?? "not started"}.</div>
                    ...
                    <div className="success-box">
                      Signed by {signedDocument?.typed_legal_name ?? signature?.typed_legal_name ?? "employee"} on{" "}
